...
CKEDITOR.plugins.add( 'timestamp', {
	...
	init: function( editor ) {
      ...
      var init_create_sheet = function(sheet_pages, init_save){
         ...
         */
         var last_sheet= sheet_pages.find(".sheet-container:last").attr("data-sheet_id");
        
         var attribut = sheet_pages.parents("header:first").find("._editor").attr("id");
           if(attribut == 'status')
           {
              getLastSheet(last_sheet,function(data){
                console.log("data",data);
                var orginal = data.meta_value;
                if(orginal != "" && orginal != 'undefined')
                {
