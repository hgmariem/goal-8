...
class Default_Assignment extends Model
{
	...
	public function default_Assignment($post){
		...
		$this->list_order=$this->get_max_list_order()+1;
		if($this->save()){
			return $this->id;
		}else{
			return false;
	...
	public function defaultAssignments(){
		$get =  Self::where(['is_delete'=>0])->orderBy("list_order", "DESC")->get();
		if($get){
			return $get;
		}
		else{
	...
	public function get_max_list_order(){
		return Self::where("is_delete",0)->max('list_order');
	}
}
