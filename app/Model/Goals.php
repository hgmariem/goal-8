...
/*
   public function add_goals($post){
	...
	public function add_goals($data){
		...
		/*$goal=Self::where('auto_save_id',$data['auto_save_id'])->get()->first();
		...
		public function add_goalsForApi($data){
	...
	public function get_goal_by_autosave($auto_save_id){
		...
		$goal=Self::where('auto_save_id',$auto_save_id)->get()->first();
		if($goal){
			return $goal;	
		}
	...
	public function get_goal($goal_id){
		$goal =  Self::where(["is_active"=>1, "is_delete"=>0,'id'=>$goal_id])->get()->first();
	...
	{
		$default_filter=['is_delete'=>0, "parent_id"=>0, "top_parent_id"=>0, "type_id"=>Config::get('constants.goal_undefined'), "user_id"=>$user_id, "is_default"=>0];
		$result = Self::where($default_filter)->orderBy("list_order", "DESC")->get(['id','name']);
		return $result;
	}
	...
	public function _get_goal_details($goal_id,$user_id){
		//echo $goal_id;die;
		$data = array();
		$goal =  Self::where(["is_delete"=>0,'id'=>$goal_id])->get()->first();
	...
	public function get_goalSheet($goal_id)
	{

		$attributes=array();
		$goal = Goals::where(["id"=>$goal_id,"parent_id"=>0,"top_parent_id"=>0,"is_delete"=>0])->get()->first();
		...
		public function _get_goal_attribut($goal_id,$user_id){

		$goal = $this->_get_goal_details($goal_id,$user_id);
		...
		$top_goal_name = '';
		$top_goal_id = '';
		if(isset($goal) && !empty($goal->top_parent_id)){
			$top = $this->_get_goal($goal->top_parent_id);
			$top_goal_name = $top->name;
			$top_goal_id = $top->id;
			$top_goal_is_active = $top->is_active;
			$top_goal_autosaveid = $top->auto_save_id;
		}
		
		$habit = $this->get_habit_types($goal->id);

		$goal->habit_type = isset($habit->habit_type)?$habit->habit_type:1;
		$goal->value = isset($habit->value)?$habit->value:7;
		$goal->lowest = isset($habit->minimum)?$habit->minimum:"";
		$goal->highest = isset($habit->maximum)?$habit->maximum:"";
		$goal->is_apply = isset($habit->is_apply)?$habit->is_apply:0;
		$goal->scale_type = isset($habit->is_scale)?$habit->is_scale:0;
		$goal->add_text_type = isset($habit->text)?$habit->text:"";	
		$goal->top_goal_name = 	$top_goal_name;
		$goal->top_goal_id = 	$top_goal_id;
		$goal->top_goal_is_active = isset($top_goal_is_active)?$top_goal_is_active:"";
		$goal->top_goal_autosaveid = isset($top_goal_autosaveid)?$top_goal_autosaveid:"";
		return $goal;
	}
	...
	public function getChildByParentId($id){
		$result = Self::where('parent_id',$id)->where('is_delete',0)->get();
		if(isset($result) && !empty($result)){
			foreach ($result as $key => $va) {
				$habit_type = HabitTypes::where("goal_id", $va->id)->get()->first();
				if(isset($habit_type) && !empty($habit_type)){
					$result[$key]["habit_type"]= $habit_type->type;
			        $result[$key]["value"]=$habit_type->value;
			        $result[$key]["lowest"]=(isset($habit_type->minimum) && !empty($habit_type->minimum))?$habit_type->minimum:0;
			        $result[$key]["highest"]=(isset($habit_type->maximum) && !empty($habit_type->maximum))?$habit_type->maximum:0;
			        $result[$key]["is_apply"]= (isset($habit_type->is_apply) && !empty($habit_type->is_apply))?$habit_type->is_apply:0;
			        $result[$key]["scale_type"]= (isset($habit_type->is_scale) && !empty($habit_type->is_scale))?$habit_type->is_scale:0;
			        $result[$key]["add_text_type"]= (isset($habit_type->add_text_type) && !empty($habit_type->add_text_type))?$habit_type->add_text_type:"";
				}else{
					$result[$key]["habit_type"]= "";
			        $result[$key]["value"]="";
			        $result[$key]["lowest"]="";
			        $result[$key]["highest"]="";
			        $result[$key]["is_apply"]= "";
			        $result[$key]["scale_type"]= "";
			        $result[$key]["add_text_type"]= "";
				}
	...
	public function change_state($auto_save_id, $self=true){
		...
		$goal =  Self::where(["is_active"=>1, "is_delete"=>0, 'auto_save_id'=>$auto_save_id])->get()->first();	 
    ...
    public function get_by_id($id){
		$get =  Self::where(["is_active"=>1, "is_delete"=>0, 'id'=>$id])->get()->first();
		if($get){
			return $get;
		}
		else{
	...
	public static function get_type_by_id($type_id){
		return $type=Types::where("id",$type_id)->get()->first();
	}	
	
    public static function get_habit_types($goal_id){
		
		$data=HabitTypes::where("goal_id", $goal_id)->get()->first();
		//var_dump($data);
		return $data;
	}
	...
	public function is_parents_deleted($parent_id, $top_parent_id){
		...
		if(!empty($ids)){
			...
			$goal =  Self::whereIn("id",$ids)->where("is_delete",1)->orderByRaw("self_order ASC")->get()->first();
	...
	public function get_habits_forAPI($filter){
		...
		if($_habits && !empty($_habits)){
			foreach ($_habits as $key => $habit) {
				...
				if(!$this->is_parents_deleted($habit->parent_id,$habit->top_parent_id)){
					...
					//if($this->validate_item($_filter)){
						if($filter['isMobile']){ // single day log for mobile view
							$habit->days=$this->weekday_logs_forAPI($habit->id, $filter['start_date']);
						}else{
							$habit->days=$this->weekdays_logs($habit->id, $filter['start_date']);
						}
	...
	public function get_tasksForApi($filter){
			// echo "<pre/>";
			// print_r($filter);die;
		if(isset($filter['view_type']) && $filter['view_type']=='list'){
			$tasks=$this->get_task_list($filter);
			...
			$tasks=$this->get_task_treeForApi($filter);
		}
	...
	public function get_task_treeForApi($filter){
		...
		//exit();
		//echo "<pre/>";
		$tasks['items']=$this->process_task_treeForApi($tasks,$filter);

		foreach ($tasks['items'] as $top_parent => $items) {
                if(is_array($items) && !empty($items) ){
                	//foreach ($items as $key => $val) {
		                $items=array_values(array_sort($items, function ($value) {
		                    return $value['self_order'];
		                }));
		                ...
		                foreach ($items as $key => $task) {
		                	...
		                	if(isset($task['children']) && !empty($task['children'])){
		                		...
		                		$task['children'] = $this->sort_children($task['children']);
		                	}
        		...
        		//array_push($new_tasks,$items);
    }

    for ($i=0; $i < sizeof($new_tasks)-1; $i++) { 
      for ($j=0; $j<sizeof($new_tasks)-1-$i; $j++) 
        if ($new_tasks[$j]['self_order'] > $new_tasks[$j+1]['self_order']) { 
          $tmp = $new_tasks[$j]; 
          $new_tasks[$j] = $new_tasks[$j+1]; 
          $new_tasks[$j+1] = $tmp; 
      } 
  }
      		$data = array_reverse($new_tasks);

		return $data;
	}


	public function sortChild($post)
	{
		$new_data = array();
		for ($i=0; $i < sizeof($post)-1; $i++) { 
      	for ($j=0; $j<sizeof($post)-1-$i; $j++) 
        if ($post[$j]['self_order'] > $post[$j+1]['self_order']) { 
          $tmp = $post[$j]; 
          $post[$j] = $post[$j+1]; 
          $post[$j+1] = $tmp; 
      		} 
	...
	public function sort_children($post){
		$new_data = array();
		foreach ($post as $key => $data) {
			if(isset($data['children']) && !empty($data['children'])){
				$data['children'] = $this->sort_children($data['children']);
			}

			$new_data[] = $data;
		}

		$new_arr = $this->sortChild($new_data);
		return $new_arr;
		...
		public function process_task_treeForApi($goals, $filter){
		...
		$tasks=$this->get_all_active_tasksForApi($parents);
...
*/
		...
		public function get_all_active_tasksForApi($goal_ids){
			...
			foreach ($goal_ids as $key => $id) {
				...
				if($goal){
					...
					if($goal['type_id']==Config::get('constants.goal_task')){
						...
						//echo "i m here...";
						$tasks[]=$this->recursive_tasks($id);
					}
				}
				

				//array_push($tasks, $goal[0]);	
			}
		...
		// 			if(is_array($val))
		// 			{
		// 				array_push($new_tasks,$val);
		// 			}
		// 			else
		// 			{
		// 				array_push($new_tasks,$value);
		// 				break;
		// 			}
	...
	public function recursive_parent($filter){
		...
		$goal =  Self::where($default_filter)->where("percent","<",100)->orderByRaw("self_order ASC")->get()->first();
	...
	public function weekday_logs_forAPI($goal_id, $start_date=null){

		$start_date=($start_date==null)?date("Y-m-d"):$start_date;

		$_weekdays=$this->weekday_forApi($start_date);
	...
	public function weekday_forApi($date){
	...
	public function get_completed_month_dates_for_percentage_scale_number($goal_id,$scale,$min,$max, $dates, $count=0){
		...
		public function get_completed_month_dates_for_log($goal_id){
		...
		$log_date = array();
		$log_value = array();
		$logs=Logs::where("goal_id",$goal_id)->where("is_scale",1)->get();

		foreach ($logs as $key => $log) {
			array_push($log_date, $log->date."-".$log->value);
		}
		$result['log_date_value'] = $log_date;
		return $result;
	}
	...
	public function get_completed_days_for_Graph_scale_number($goal_id,$scale,$dates){
		...
		$logs=Logs::where("goal_id",$goal_id)->where('is_scale',$scale)->where("date",$dates)->get()->first();
		$value = 0;
		if($logs){
			$value = $logs->value;
		}
	...
	public function get_additional_completed_dates_scale_number($goal_id=0,$scale=0,$min=0,$max=0,$habit_date=0,$current_date=0,$count=0){
		...
		$logs=Logs::where("goal_id",$goal_id)->where('is_scale',$scale)->where(function($q) use ($min,$max) {
          $q->where('value','>=',$min)
            ->where('value','<=',$max)
            ->orWhere('value',0);
      })->whereRaw("date < '".$current_date."'")->whereRaw("date >= '".$habit_date."'")->get();
	...
	public function get_disabled_days_scalar_number($goal_id, $dates, $count=0){
		...
		$logs=Logs::where("goal_id",$goal_id)->where('is_scale',1)->where("value",-1)->whereIn("date", $dates)->get();
	...
	public function get_aditional_days_scalar_number($goal_id,$min,$max,$dates, $count=0){
		...
		$logs=Logs::where("goal_id",$goal_id)->where('is_scale',1)->where(function($q) use ($min,$max) {
          $q->where('value','>=',$min)
            ->where('value','<=',$max)
            ->orWhere('value',0);
	...
	public function get_log($id, $date){
		return Logs::where("goal_id",$id)->where("date",$date)->get()->first();
	}

	public function get_first_log_date($id){
		return Logs::where("goal_id",$id)->orderBy("date", "ASC")->get()->first();
	}
	...
	public function get_percentage($habit_id=null){
		...
		var_dump($type_data);
		exit();*/
		//echo var_dump($type_data->is_scale);die;
		if(isset($type_data) && $type_data->is_scale == 0)
		{
			//echo "if";die;
			 $days_per_week=($type_data &&$type_data->count_per_week!='')?$type_data->count_per_week:7;
			
			/*echo "<pre/>";
			print_r($days_per_week);die;*/
		...
		$response['created_at']=$habit->created_at;
		...
		else if(isset($type_data) && $type_data->is_scale == 1)
		{
			//echo "else";die;
			$days_per_week=($type_data &&$type_data->count_per_week!='')?$type_data->count_per_week:7;
		...
		if($days_per_week!=7){
			...
			if($day_numbers){
				...
				$dayOfTheWeek = Carbon::parse($habit->habit_start_date)->dayOfWeek;
					...
					//$startDate = Carbon::parse($habit->habit_start_date); // Get the first friday.
				$startDate = date('Y-m-d', strtotime($habit->habit_start_date));
					$endDate = date('Y-m-d');
					...
					//for ($date = $startDate; $date->lte($endDate)) {
						...
						if(in_array($day_of_week, $day_numbers)){
							$applicable_dates[] = $startDate;
						}else{
							$not_applicable_dates[] = $startDate;
						}
		...
		$disabled_scalar_days = $this->get_disabled_days_scalar_number($habit->id, $applicable_dates, 1);

		
		$addtional_scalar_days = $this->get_aditional_days_scalar_number($habit->id,$type_data->minimum,$type_data->maximum, $not_applicable_dates, 1);

		$disabled_days=$this->get_disabled_dates_scale_number($habit->id,$type_data->is_scale,$habit->habit_start_date, date("Y-m-d"), 1);
		
		  $days = ($days-$disabled_scalar_days);
		}
	...
	public function get_monthly_percentage($habit_id, $date){
		...
		for($i = 1; $i <= $length; $i++){
			...
			if($loop_date >= $habit->created_at){
				...
				if($value !=7 && in_array($day_of_week, $allowed_days)){
					$applicable_dates[$loop_date] = "'".$loop_date."'";
				}else if($value != 7){
					$not_applicable_dates[$loop_date] = "'".$loop_date."'";
				}else if($value == 7){
		...
		$habit = Goals::where('id',$habit_id)->get()->first();
		//var_dump($length);
		$type_data = Self::get_habit_types($habit->id);
	...
	public function weeklyGraphDetails($habit_id,$start_date,$end_date)
	{
		$habit = Goals::where('id',$habit_id)->get()->first();
		...
		while (strtotime($start_date) <= strtotime($end_date)) {
		    ...
		    $weekEndDate = date ("Y-m-d", strtotime("+6 day", strtotime($weekStartDate)));

		  if($weekStartDate >= $habit->habit_start_date){
		  	while ($weekStartDate <= $weekEndDate) {
				$day_of_week = Carbon::parse($weekStartDate)->dayOfWeek;
					
				if($type_data->value !=7 && in_array($day_of_week, $allowed_days)){
					$applicable_dates[$weekStartDate] = "'".$weekStartDate."'";
				}else if($type_data->value != 7){
					$not_applicable_dates[$weekStartDate] = "'".$weekStartDate."'";
				}else if($type_data->value == 7){
					$applicable_dates[$weekStartDate] = "'".$weekStartDate."'";
				}
				 $weekStartDate = date ("Y-m-d", strtotime("+1 day", strtotime($weekStartDate)));	
		  	}
		   ...
		   $days = 7-count($not_applicable_dates);
		   ...
		   $scale_additional_days = $this->get_additional_completed_month_dates_for_percentage_scale_number($habit->id,$type_data->is_scale,$type_data->minimum,$type_data->maximum, $not_applicable_dates);
			...
			$weekly_lowest[$i] = $highest_lowest['lowest'];
			$weekly_highest[$i] = $highest_lowest['highest'];
		    $weekly_tot[$i] = $scale_completed;
			$weekly_aver[$i] = round($monthly_average);
		 ...
		 $response['lowest'] = $weekly_lowest;
		 $response['highest'] = $weekly_highest;
		 $response['weekly_tot'] = $weekly_tot;
		 $response['weekly_aver'] = $weekly_aver;
		 $response['labels']       = $labels;
		 $response['xAxes'] = "weekly";
	...
	public function dayByHabitIdAverage($habit_id,$start_date,$end_date)
	{
		...
		while (strtotime($start_date) <= strtotime($end_date)) {
			...
			$day_of_week = Carbon::parse($start_date)->dayOfWeek;
				
			if($type_data->value !=7 && in_array($day_of_week, $allowed_days)){
				$applicable_dates[$start_date] = $start_date;
			}else if($type_data->value != 7){
				$not_applicable_dates[$start_date] = $start_date;
			}else if($type_data->value == 7){
				$applicable_dates[$start_date] = $start_date;
			}			
		}
	...
	foreach ($applicable_dates as $key => $date)
	 {
			$value = $this->get_completed_days_for_Graph_scale_number($habit_id,$type_data->is_scale,$date);
			if($value != -1)
			{
	...
	public function getMonthScaleAndTotal($habit_id,$start_date,$end_date)
	{
		...
		while(strtotime($start_month) <= strtotime($end_month))
		{
			...
			$days = 0;
			    ...
			    //$monthly_average = $scale_completed/$days;
				$month_lowest[$i] = (isset($highest_lowest['lowest']) && !empty($highest_lowest['lowest']))?$highest_lowest['lowest']:0;
				$month_highest[$i] = (isset($highest_lowest['highest']) && !empty($highest_lowest['highest']))?$highest_lowest['highest']:0; 
...
public function getYearScaleAndTotal($habit_id,$start_date,$end_date)
		...
		if($habitMonth == $current)
		{
			...
			//$addtional_days=0;
				...
				else
				{
					...
					$response['completed_days'] = $completed;
					$scale_completed_log_days = $this->get_completed_month_dates_for_log($habit->id);
					$response['log_date_value'] = $scale_completed_log_days['log_date_value'];
				    ...
				    //$monthly_average = $scale_completed/$days;
					if(!empty($highest_lowest['highest']))
				    {
				    	$highest = $highest_lowest['highest'];
				    }

				    if(!empty($highest_lowest['lowest']))
				    {
	...
	public function generate_autosaveid(){
		$Random_number = str_pad(rand(0, pow(10, 5)-1)*99999, 5, '0', STR_PAD_LEFT);
		do {
			
			$tbl_goals = Goals::where('auto_save_id', '=', $Random_number)
				->get();
		  }
		  while ($tbl_goals == null);
                {
					return	$Random_number;
				}
		/* $sql='SELECT '.$number.' AS random_num FROM tbl_goals WHERE "random_num" NOT IN (SELECT auto_save_id FROM tbl_goals) LIMIT 1';
		$results = DB::select( DB::raw($sql));
		if($results){
			return $results[0]->random_num;
		}
		return $sql; */
		//print_r($results);
	}
	...
	public function validate_item($filter){
		$goal =  Self::whereRaw("top_parent_id=".$filter['top_parent_id']." OR parent_id=".$parent_id)->orderByRaw("self_order ASC")->get()->first();
		if($goal){
			return $goal;
		}
		return false;
		...
		public function addLobby($post,$user_id)
			...
			else
			{
			 ...
			 if($post['type'] == 2 && isset($post['task_name']) && !empty($post['task_name'])){
			 	$data['has_sub'] = 1;
			 }
	...
	public static function getBreadcum($goal_id,$parent_id){
		$data = array();

		if(isset($goal_id) && !empty($goal_id) && isset($parent_id) && !empty($parent_id)){
		$goal = Self::where('id',$goal_id)->where('is_delete',0)->get()->first();
		
		if(isset($goal) && !empty($goal) && $goal->parent_id == $goal->top_parent_id){
		$topGoal = Self::where('id',$goal->parent_id)->where('is_delete',0)->get()->first();
		if(isset($topGoal) && !empty($topGoal)){
			$data['top_goal'] = $topGoal->name;
			$data['top_id'] = $topGoal->id;
			$data['parent_goal'] = "";
			$data['parent_id'] = "";
			$data['middile_goal'] = "";
			$data['middile_goal_id'] = "";
			$data['next_parent'] = 0;
			}else{
			$data['top_goal'] = "";
			$data['top_id'] = "";
			...
			$topGoal = Self::where('id',$goal->top_parent_id)->where('is_delete',0)->get()->first();
			$data['top_goal'] = $topGoal->name;
			$data['top_id'] = $topGoal->id;
			$parentGoal = Self::where('id',$goal->parent_id)->where('is_delete',0)->get()->first();
			if(isset($parentGoal) && !empty($parentGoal)){
				$middileGoal = Self::where('id',$parentGoal->parent_id)->where('is_delete',0)->get()->first();
				$data['parent_goal'] = $parentGoal->name;
				$data['parent_goal_id'] = $parentGoal->id;
				if(isset($middileGoal) && !empty($middileGoal) && $middileGoal->id != $topGoal->id){
					$data['middile_goal'] = $middileGoal->name;
					$data['middile_goal_id'] = $middileGoal->id;
					if(isset($middileGoal) && !empty($middileGoal) && $middileGoal->parent_id == $middileGoal->top_parent_id){
					 $data['next_parent'] = 0;
					}else{
						$data['next_parent'] = 1;
					}
				...
				if(isset($topGoal) && !empty($topGoal)){
					$data['top_goal'] = $topGoal->name;
					$data['top_id'] = $topGoal->id;
					$data['parent_goal'] = $goal->name;
					$data['parent_id'] = $goal->id;
					$data['middile_goal'] = "";
					$data['middile_goal_id'] = "";
					$data['next_parent'] = 0;
					}else{
					...
					if(isset($parentGoal) && !empty($parentGoal)){
						...
						$data['parent_goal'] = $goal->name;
						$data['parent_goal_id'] = $goal->id;
						$data['middile_goal'] = $parentGoal->name;
						$data['middile_goal_id'] = $parentGoal->id;
						  if(isset($middileGoal) && !empty($middileGoal) && $parentGoal->parent_id == $parentGoal->top_parent_id){
							 $data['next_parent'] = 0;
						   }else{
								$data['next_parent'] = 1;
						   }
