...
?>
<ul class="task-list task-main-tree task-list-leaf" id="task"> 
    ...
    ?>
    <li class="<?php echo $_task->list_collapse?"child-expand":"child-collapse"; ?> task-item task-desk-leaf" id="item_<?php echo $_task->id?>" data-order="<?php echo $_task->self_order?>">
                        ...
                        <div class="task-link btn" data-id="<?php echo $_task->id?>" data-collapse="">
                            <span class="goal-data">
                                <span class="gd-heading">
                                    <?php $task_name=mb_convert_encoding(_substr($_task->name,0, 120), 'UTF-8', 'UTF-8');?>
                                    <a class="task-link-title" href="{{URL('edit/'.$_task->top_parent_id.'#'.$_task->id)}}" title="<?php echo $_task->name?>">
                                        <?php echo $task_name?> 
                                    </a>
                                </span>
                                <span class="gd-date">
